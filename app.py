...
def create_initial_ratings():
    ...
    if st.button("Сохранить оценки") and len(ratings) >= 3:
        pd.DataFrame(ratings).to_csv("personal_ratings_detailed.csv", index=False)
...
def add_single_rating():
    ...
    if not top_movies:
        return
    rated_ids = set()
    if os.path.exists("personal_ratings_detailed.csv"):
        rated_ids = set(pd.read_csv("personal_ratings_detailed.csv")["movieId"].astype(int))
    ...
    if st.button("Сохранить"):
        ...
        if os.path.exists("personal_ratings_detailed.csv"):
            existing = pd.read_csv("personal_ratings_detailed.csv")
            existing = pd.concat([existing, df], ignore_index=True)
            existing.to_csv("personal_ratings_detailed.csv", index=False)
        else:
            df.to_csv("personal_ratings_detailed.csv", index=False)
        st.success("Оценка сохранена.")
        if len(pd.read_csv("personal_ratings_detailed.csv")) >= 3:
            retrain_model()
        st.rerun()
...
def main():
    ...
    if page == "top":
        ...
        if not os.path.exists("personal_ratings_detailed.csv"):
            ...
            with tabs[0]:
                df = pd.read_csv("personal_ratings_detailed.csv")
